...
def weak_scaling():
    ...
    for p in [1, 2, 4]:
        ...
        gflops = flops / dt / 1e9
        results.append(
            {"p": p, "N": N, "time": dt, "gflops": gflops, "intensity": intensity}
        )
    return results
...
def io_benchmark():
    ...
    for size in sizes:
        ...
        results.append(
            {
                "size": size,
                "write_MBps": bytes_ / write_t / 1e6,
                "read_MBps": bytes_ / read_t / 1e6,
            }
        )
    return results
...
def plot_scaling(strong, weak, out_dir):
    ...
    plt.savefig(out_dir / "strong_scaling.png")
    plt.close()
    ...
    plt.savefig(out_dir / "weak_scaling.png")
    plt.close()
...
def plot_io(io, out_dir):
    ...
    plt.figure()
    plt.plot(sizes, w, marker="o", label="write")
    plt.plot(sizes, r, marker="s", label="read")
    plt.xlabel("Size (MB)")
    plt.ylabel("Throughput (MB/s)")
    plt.title("I/O Benchmark")
    plt.legend()
    plt.savefig(out_dir / "io_benchmark.png")
    plt.close()
