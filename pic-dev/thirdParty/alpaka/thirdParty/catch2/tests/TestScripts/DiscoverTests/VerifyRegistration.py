...
def build_project(sources_dir, output_base_path, catch2_path):
    ...
    try:
        subprocess.run(config_cmd, capture_output=True, check=True, text=True)
        subprocess.run(build_cmd, capture_output=True, check=True, text=True)
...
if __name__ == "__main__":
    ...
    catch_test_names = [
        escape_catch2_test_name(name) for name in get_test_names(build_path)
    ]
    ...
    if mismatched:
        print(
            f"Found {mismatched} mismatched tests catch test names and ctest test commands!"
        )
        exit(1)
