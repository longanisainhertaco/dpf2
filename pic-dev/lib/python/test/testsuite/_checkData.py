...
def checkDirection(
    variable: str = "undefined", direction: str = None, errorhandling: bool = False
) -> str:
    """
    ...
    if val is not None and direction is not None:
        warnings.warn(
            "Both "
            + variable
            + " and direction are set.Note that the value from config.py is used."
        )
        ...
        raise ValueError(
            "Both "
            + variable
            + " and direction are none.You must set at least one value."
        )
...
def checkVariables(variable: str = "undefined", default=None, parameter=None):
    ...
    if val is not None and parameter is not None:
        warnings.warn(
            "Both "
            + val_name
            + " and the optional parameter are set. Note that the value from config.py is used."
        )
