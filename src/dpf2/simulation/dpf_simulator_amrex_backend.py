...
class DPFSimulatorAMReXBackend(DPFSimulatorBackend):
    def __init__(self, config: dict, field_manager: FieldManager):  # Add field_manager
        ...
        self.telemetry_vars = config.get("telemetry", {}).get(
            "variables",
            [
                "I",
                "V",
                ...
                "divB_max",
                "divB_l2",
                "cumulative_neutron_count",
                "neutron_rate",
                "amr_levels",
                "dt",
                "cell_balance_min",
                "cell_balance_max",
                "particle_balance_min",
                "particle_balance_max",
                "cpu_mem_usage_mb",
            ],
        )
        ...
        self.coarse_plotfile_variables = io_cfg.get(
            "coarse_plotfile_variables", ["rho", "B"]
        )
        self.fine_plotfile_variables = io_cfg.get(
            "fine_plotfile_variables", ["rho", "B", "mom", "energy"]
        )
        ...
        self.bp_engine = self.bp_io.Open(
            io_cfg.get("telemetry_stream", "telemetry.bp"), adios2.Mode.Write
        )
        ...
        self.diagnostics = Diagnostics(
            ...
            config={
                **config["circuit_params"],
                **(
                    config["collision_params"] if config.get("collision_params") else {}
                ),
                **(
                    config["radiation_params"] if config.get("radiation_params") else {}
                ),
                **(config["pic_params"] if config.get("pic_params") else {}),
                **(config["hybrid_params"] if config.get("hybrid_params") else {}),
            },
    ...
    def run(self):
        ...
        try:
            ...
            self.state = SimulationState(
                ...
                domain_lo=self.domain_lo,
                boundary_conditions=(
                    self.config.geometry.boundary_conditions
            ...
            if self.mode in ("fluid", "hybrid"):
                ...
                self.fluid_solver.field_manager = (
                    self.field_manager
                )  # Ensure FieldManager is set
            if self.mode in ("pic", "hybrid"):
            ...
            if self.mode == "hybrid":
                ...
                while self.t < self.sim_time:
                    ...
                    if self.mode == "fluid":
                        ...
                        self.circuit.step_trapezoidal(
                            self.state, dt
                        )  # Assuming this is correct for PIC
                    self._post_step(dt)
                    self.t += dt
    ...
    def _post_step(self, dt=None):
        try:
            ...
            self.diagnostics.record(
                self.t, self.circuit, self.fluid_solver, self.pic_solver, self.radiation
            )
            ...
            Ldyn = (
                self.diagnostics.get_latest()["mode_amplitudes"]
                if self.diagnostics.get_latest()
                and "mode_amplitudes" in self.diagnostics.get_latest()
                else {}
            )
            tel["dynamic_inductance"] = Ldyn
            tel["pinch_flag"] = (
                int(self.diagnostics.get_latest()["dI_dt"] < 0)
                if self.diagnostics.get_latest()
                and "dI_dt" in self.diagnostics.get_latest()
                else 0
            )
            # sheath kinematics
            ...
            tel["energy_residual"] = (
                self.diagnostics.get_latest()["E_radiated"]
                if self.diagnostics.get_latest()
                and "E_radiated" in self.diagnostics.get_latest()
                else 0.0
            )
            # divergence errors
            ...
            tel["divB_l2"] = divB_l2
            # neutron yield
            tel["cumulative_neutron_count"] = (
                sum(self.diagnostics.get_latest()["histogram"])
                if self.diagnostics.get_latest()
                and "histogram" in self.diagnostics.get_latest()
                else 0
            )
            # AMR & dt
            ...
            if self.openpmd_writer and self.t >= self.next_openpmd:
                ...
                particles = {}
                if self.mode != "fluid" and hasattr(
                    self.pic_solver, "get_particle_data"
                ):
            ...
            if self.t >= self.next_coarse:
                self.fluid_solver.write_plotfile(
                    levels="coarse",
                    metadata=self.provenance,
                    variables=self.coarse_plotfile_variables,
                )
                if self.mode != "fluid":
                    self.pic_solver.write_plotfile(
                        levels="coarse",
                        metadata=self.provenance,
                        variables=self.coarse_plotfile_variables,
                    )
            ...
            if self.t >= self.next_highres:
                self.fluid_solver.write_plotfile(
                    levels="fine",
                    metadata=self.provenance,
                    variables=self.fine_plotfile_variables,
                )
                if self.mode != "fluid":
                    self.pic_solver.write_plotfile(
                        levels="fine",
    ...
    def _checkpoint(self, amrex_restart=False):
        if amrex_restart:
            ...
            if self.mode != "fluid":
                self.pic_solver.write_checkpoint(restart=True)
        else:
            super()._checkpoint()
