...
class ModuleRegistry:
    ...
    def __init__(self):
        self.modules: Dict[Type, Dict[str, Any]] = (
            {}
        )  # Class -> {config_schema, metadata, dependencies}
    ...
    def register(
        ...
        field_manager_required: bool = False,
        **metadata,
    ):
        """
        ...
        if not issubclass(module_class, PhysicsModule):
            raise TypeError(
                f"{module_class.__name__} must be a subclass of PhysicsModule."
            )
    ...
    def discover_plugins(self, package_name: str):
        ...
        * Import errors are logged and the offending modules are skipped.
        ...
        try:
            ...
            logger.error(
                f"Error during plugin discovery in package {package_name}: {e}"
            )
    ...
    def create(
        ...
        created_modules: Optional[Dict[Type, Any]] = None,
        ...
        if module_class not in self.modules:
            raise ValueError(
                f"Unknown module: {module_class.__name__}. Please ensure the module is registered."
            )
        ...
        for dep_class in dependencies:
            if dep_class not in created_modules:
                logger.info(
                    f"Creating dependency {dep_class.__name__} for {module_class.__name__}"
                )
                created_modules[dep_class] = self.create(
                    dep_class,
                    config=None,
        ...
        if config and config_schema:
            try:
                ...
                raise ModuleConfigurationError(
                    f"Invalid configuration for module {module_class.__name__}:\n{error_messages}"
                )
        ...
        try:
            ...
            if self.modules[module_class]["field_manager_required"]:
                if field_manager is None:
                    raise ModuleDependencyError(
                        f"Module {module_class.__name__} requires a FieldManager."
                    )
                if config:
            ...
            raise ModuleInitializationError(
                f"Error instantiating module {module_class.__name__}: {e}"
            )
    ...
    def initialize_module(self, module: Any):
        ...
        if hasattr(module, "initialize"):
            try:
                ...
                raise ModuleInitializationError(
                    f"Error initializing module {module.__class__.__name__}: {e}"
                )
    ...
    def finalize_module(self, module: Any):
        ...
        if hasattr(module, "finalize"):
            try:
                ...
                raise ModuleInitializationError(
                    f"Error finalizing module {module.__class__.__name__}: {e}"
                )
