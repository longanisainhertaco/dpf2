...
def weno5_reconstruct(u, eps, gamma0, gamma1, gamma2, nx, flux):
    ...
    for i in prange(2, nx - 2):
        # stencils
        b0 = (13 / 12) * (u[i - 2] - 2 * u[i - 1] + u[i]) ** 2 + (1 / 4) * (
            u[i - 2] - 4 * u[i - 1] + 3 * u[i]
        ) ** 2
        b1 = (13 / 12) * (u[i - 1] - 2 * u[i] + u[i + 1]) ** 2 + (1 / 4) * (
            u[i - 1] - u[i + 1]
        ) ** 2
        b2 = (13 / 12) * (u[i] - 2 * u[i + 1] + u[i + 2]) ** 2 + (1 / 4) * (
            3 * u[i] - 4 * u[i + 1] + u[i + 2]
        ) ** 2
        a0 = gamma0 / ((eps + b0) ** 2)
        a1 = gamma1 / ((eps + b1) ** 2)
    ...
    for i in prange(nx - 1):
        for j in range(ny - 1):
            for k in range(nz - 1):
                ...
                if len(stencil) != 5:
                    logger.warning(
                        f"Invalid stencil size at ({i}, {j}, {k}): {len(stencil)}"
                    )
                    continue

                # WENO5 reconstruction
                b0 = (13 / 12) * (stencil[0] - 2 * stencil[1] + stencil[2]) ** 2 + (
                    1 / 4
                ) * (stencil[0] - 4 * stencil[1] + 3 * stencil[2]) ** 2
                b1 = (13 / 12) * (stencil[1] - 2 * stencil[2] + stencil[3]) ** 2 + (
                    1 / 4
                ) * (stencil[1] - stencil[3]) ** 2
                b2 = (13 / 12) * (stencil[2] - 2 * stencil[3] + stencil[4]) ** 2 + (
                    1 / 4
                ) * (3 * stencil[2] - 4 * stencil[3] + stencil[4]) ** 2
                a0 = gamma0 / ((eps + b0) ** 2)
                a1 = gamma1 / ((eps + b1) ** 2)
                a2 = gamma2 / ((eps + b2) ** 2)
                s = a0 + a1 + a2
                w0, w1, w2 = a0 / s, a1 / s, a2 / s
                flux[i, j, k] = (
                    w0 * (2 * stencil[0] - 7 * stencil[1] + 11 * stencil[2])
                    + w1 * (-stencil[1] + 5 * stencil[2] + 2 * stencil[3])
                    + w2 * (2 * stencil[2] + 5 * stencil[3] - stencil[4])
                ) / 6.0
...
class FluidSolverHighOrder:
    ...
    def __init__(self, geom, config, field_manager: FieldManager):
        try:
            ...
            self.coll = CollisionModel(
                config.get("collision_cfg", None)
            )  # Instantiate CollisionModel
    ...
    def step(
        ...
        try:
            ...
            U_star = {
                k: U0[k] + dt * (1 - self.config["imex_alpha"]) * rhs[k]
                for k in U0
                if k in rhs
            }
            ...
            self.state["reconnection_rate"] = MultiFab(
                self.state["density"].boxArray(),
                self.state["density"].DistributionMap(),
                1,
                0,
            )
    ...
    def _compute_explicit_rhs(self, U):
        try:
            ...
            eta_anom = (
                np.maximum(
                    0.0,
                    (np.linalg.norm(J, axis=3) - self.config["Jcrit"])
                    / self.config["Jcrit"],
                )
                * self.config["eta0"]
            )
            ...
            for i in range(3):
                div_tau[..., i] = eta_par * (
                    np.gradient(
                        np.gradient(v[..., i], self.dx, axis=0), self.dx, axis=0
                    )
                    + np.gradient(
                        np.gradient(v[..., i], self.dy, axis=1), self.dy, axis=1
                    )
                    + np.gradient(
                        np.gradient(v[..., i], self.dz, axis=2), self.dz, axis=2
                    )
                )
            gradTe = np.stack(np.gradient(Te, self.dx, axis=0), axis=3)
            Bhat = np.where(Bmag[..., None] > 0, B / Bmag[..., None], 0.0)
            q_par = -kappa_par[..., None] * (
                Bhat * np.sum(gradTe * Bhat, axis=3)[..., None]
            )
            q_per = -kappa_per[..., None] * (
                gradTe - Bhat * np.sum(gradTe * Bhat, axis=3)[..., None]
            )
            q = q_par + q_per
            div_q = sum(
                np.gradient(q[..., i], [self.dx, self.dy, self.dz][i], axis=i)
                for i in range(3)
            )
            ...
            radiation_loss_term = self.radiation_model.compute_radiation_loss(
                {
                    ...
                    "Z": 1.0,
                    "Bmag": Bmag,
                }  # Example radiation loss
            )
            self.state["viscosity"].setVal(viscosity_term)
            self.state["radiation_loss"].setVal(radiation_loss_term)

            # Magnetic induction
            curlEx = np.zeros_like(rho)
            curlEy = np.zeros_like(rho)
            curlEz = np.zeros_like(rho)
            curl(
                E[..., 0],
                E[..., 1],
                E[..., 2],
        ...
        try:
            ...
            for key in ["rho", "momentum", "energy_i", "energy_e"]:
                if key == "momentum":
                    for c in range(3):
                        arr = U[key][..., c]
                        flux_x = np.zeros_like(arr)
                        flux_y = np.zeros_like(arr)
                        flux_z = np.zeros_like(arr)
                        weno5_reconstruct_3d(
                            arr,
                            eps,
                            g0,
                            g1,
    ...
    def _imex_solve(self, U, dt_imp):
        try:
            ...
            Te = pe / (rho + 1e-30)
            Bmag = np.linalg.norm(B, axis=3)
            eta_par, eta_per, kappa_par, kappa_per = braginskii_coeffs(
                ne, Te, Bmag
            )  # Compute Braginskii coefficients
            ...
            momentum_diffusion_operator = (
                amrex.MultiFabViscousOp(  # Create diffusion operator
                    self.geom,
                    eta_par,
                    beta=1.0,
            ...
            momentum_solver.solve(
                self.state["momentum"],
                self.state["momentum"],
                self.config["imex_eps"],
                self.config["imex_tol"],
    ...
    def _amr_refine(self):
        try:
            ...
            mach_number = np.linalg.norm(v, axis=3) / np.sqrt(
                self.eos.gamma * (pi + pe) / rho
            )
            ...
            magnetic_field_gradient_threshold = (
                1e4  # Example threshold for magnetic field gradient
            )
            temperature_gradient_threshold = (
                1e3  # Example threshold for temperature gradient
            )
            ...
            tags = amrex.FabArrayIO.tagCells(
                ...
                criteria={
                    ...
                    "temperature_gradient": np.linalg.norm(
                        np.gradient(Te, self.dx, axis=0), axis=3
                    ),
    ...
    def _radiation_step(self, dt):
        try:
            ...
            for g in range(self.num_groups):
                mf_phi = MultiFab(
                    self.geom.boxArray(), self.state["energy_e"].DistributionMap(), 1, 0
                )
                # diffusion operator L phi = source
                if (
                    not isinstance(self.config["rad_alpha"][g], (int, float))
                    or self.config["rad_alpha"][g] <= 0
                ):
                    raise ValueError(
                        f"Invalid rad_alpha value for group {g}: {self.config['rad_alpha'][g]}"
                    )
                linop = MultiFabLaplacian(
                    self.geom, mf_phi, alpha=self.config["rad_alpha"][g], beta=1.0
                )
                solver = MLMG(linop)
                solver.solve(
                    mf_phi,
    ...
    def _photon_monte_carlo(self, dt):
        try:
            ...
            emissivity = self.config["photon_emissivity"](
                self.state
            )  # user-provided func
            ...
            for p in range(num_packets):
                ...
                kap = opacity[i, j, k]
                L = (
                    -np.log(np.random.rand()) / (kap + 1e-30)
                    if kap > 0
                    else max(self.dx, self.dy, self.dz)
                )
                # deposit at traveled location
                dist_steps = L / np.linalg.norm([self.dx, self.dy, self.dz])
                ip = int(
                    np.clip(
                        i + dir_vec[0] * dist_steps,
                        0,
                        self.state["density"].array().shape[0] - 1,
                    )
                )
                jp = int(
                    np.clip(
                        j + dir_vec[1] * dist_steps,
                        0,
                        self.state["density"].array().shape[1] - 1,
                    )
                )
                kp = int(
                    np.clip(
                        k + dir_vec[2] * dist_steps,
                        0,
                        self.state["density"].array().shape[2] - 1,
                    )
                )
    ...
    def _reconnection_rate(self):
        try:
            ...
            if (
                self.field_manager is None
                or self.field_manager.get_E() is None
                or self.field_manager.get_B() is None
            ):
                return 0.0
            ...
            B = self.field_manager.get_B()
            return np.max(
                np.abs(np.sum(E * B, axis=3) / (np.linalg.norm(E, axis=3) + 1e-30))
            )
        except Exception as e:
