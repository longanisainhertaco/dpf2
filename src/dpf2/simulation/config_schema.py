...
class CircuitConfig(BaseModel):
    ...
    switch_resistance: float = Field(
        0.0, ge=0, description="Resistance of the switch [Ω]"
    )
    switch_inductance: float = Field(
        0.0, ge=0, description="Inductance of the switch [H]"
    )
    transmission_line_impedance: float = Field(
        50.0, gt=0, description="Impedance of the transmission line [Ω]"
    )
    transmission_line_length: float = Field(
        1.0, gt=0, description="Length of the transmission line [m]"
    )
...
class CollisionConfig(BaseModel):
    ...
    electron_neutral_collision_enabled: bool = Field(
        False, description="Enable electron-neutral collisions"
    )
    electron_neutral_cross_section: float = Field(
        1e-19, gt=0, description="Electron-neutral cross-section [m^2]"
    )
    charge_exchange_enabled: bool = Field(
        False, description="Enable charge exchange collisions"
    )
    charge_exchange_cross_section: float = Field(
        1e-19, gt=0, description="Charge exchange cross-section [m^2]"
    )
...
class RadiationConfig(BaseModel):
    ...
    line_cooling_curve: Optional[str] = Field(
        None, description="Path to the HDF5 file containing the line cooling curve"
    )
    telemetry_port: int = Field(..., description="Port for telemetry streaming")
    photon_params: Dict[str, Any] = Field(
        default_factory=dict, description="Parameters for photon Monte Carlo"
    )
    opacity_model: str = Field("constant", description="Model for opacity calculation")
    opacity_params: Dict[str, Any] = Field(
        default_factory=dict, description="Parameters for opacity model"
    )
    group_opacities: List[float] = Field(
        default_factory=list,
        description="Opacities for each radiation group",
    ...
    adios_parameters: Dict[str, Any] = Field(
        default_factory=dict, description="ADIOS2 parameters"
    )
    ...
    num_photons_per_cell: int = Field(
        10, description="Number of photons to emit per cell per time step"
    )
    bremsstrahlung_enabled: bool = Field(
        True, description="Enable Bremsstrahlung radiation"
    )
    synchrotron_enabled: bool = Field(True, description="Enable Synchrotron radiation")
    compton_scattering_enabled: bool = Field(
        False, description="Enable Compton scattering"
    )
    dynamic_opacities_enabled: bool = Field(
        False, description="Enable dynamic opacities"
    )
    photon_monte_carlo_enabled: bool = Field(
        False, description="Enable photon Monte Carlo"
    )
    eddington_tensor_closure_enabled: bool = Field(
        False, description="Enable Eddington tensor closure"
    )
    dynamic_line_radiation_enabled: bool = Field(
        False, description="Enable dynamic line radiation"
    )
    ...
    non_lte_line_transport: bool = Field(
        True,
        description="Enable non-LTE line transport with time-dependent level populations",
    )
    ...
    def _convert_material_opacities(cls, v):
        ...
        if isinstance(v, dict):
            return [{"material_id": k, "group_opacities": val} for k, val in v.items()]
        return v
...
class PICConfig(BaseModel):
    ...
    max_grid_size: int = Field(
        64, description="Maximum grid size for adaptive mesh refinement"
    )
    max_level: int = Field(0, description="Maximum AMR level")
    initial_grid_level: int = Field(0, description="Initial AMR level")
    time_step_scaling_factor: float = Field(
        1.0, description="Scaling factor for the time step"
    )
    particle_relocation_method: str = Field(
        "momentum_conserving", description="Method for particle relocation"
    )
    field_gathering_order: int = Field(
        2, description="Order of the field gathering scheme"
    )
    particle_pusher_algo: str = Field(
        "boris", description="Algorithm for pushing particles"
    )
    current_deposition_algo: str = Field(
        "esirkepov", description="Algorithm for current deposition"
    )
    field_smoothing_iterations: int = Field(
        0, description="Number of iterations for field smoothing"
    )
    field_smoothing_strength: float = Field(
        0.0, description="Strength of field smoothing"
    )
    particle_injection_style: str = Field(
        "full_box", description="Style of particle injection"
    )
    particle_injection_temperature: float = Field(
        1.0, description="Temperature of injected particles"
    )
    particle_injection_density: float = Field(
        1.0, description="Density of injected particles"
    )
    particle_injection_velocity: List[float] = Field(
        [0.0, 0.0, 0.0], description="Velocity of injected particles"
    )
    particle_injection_position: List[float] = Field(
        [0.0, 0.0, 0.0], description="Position of injected particles"
    )
    ...
    enable_mesh_adaptivity: bool = Field(
        False, description="Enable mesh adaptivity hooks"
    )
    relativistic_corrections: bool = Field(
        False, description="Apply relativistic corrections to particle motion"
    )
    quantum_emission: bool = Field(
        False, description="Enable quantum emission processes"
    )
    time_dependent_boundaries: bool = Field(
        False, description="Use time-dependent boundary fields"
    )
    # Add other parameters as needed
...
class HybridConfig(BaseModel):
    ...
    coupling_tolerance: float = Field(
        1e-6, description="Tolerance for the coupling iteration"
    )
    target_volume_fraction: float = Field(
        0.5, description="Target volume fraction for the PIC region"
    )
    transition_criteria_gradient_threshold: float = Field(
        0.1, description="Gradient threshold for transition criteria"
    )
    transition_criteria_knudsen_number_threshold: float = Field(
        0.1, description="Knudsen number threshold for transition criteria"
    )
    transition_criteria_hall_parameter_threshold: float = Field(
        0.1, description="Hall parameter threshold for transition criteria"
    )
    transition_criteria_non_maxwellianity_factor: float = Field(
        1.0, description="Non-Maxwellianity factor for transition criteria"
    )
    # Add other parameters as needed
...
class DiagnosticsConfig(BaseModel):
    ...
    field_diagnostic_interval: int = Field(
        10, description="Interval for field diagnostics"
    )
    particle_diagnostic_interval: int = Field(
        10, description="Interval for particle diagnostics"
    )
    energy_diagnostic_interval: int = Field(
        10, description="Interval for energy diagnostics"
    )
    mode_analysis_enabled: bool = Field(
        False, description="Enable mode analysis diagnostic"
    )
    thomson_scattering_enabled: bool = Field(
        False, description="Enable Thomson scattering diagnostic"
    )
    # Add other parameters as needed
...
class GeometryConfig(BaseModel):
    ...
    domain_length: List[float] = Field(
        ..., description="Length of the simulation domain in each dimension"
    )
    domain_width: List[float] = Field(
        ..., description="Width of the simulation domain in each dimension"
    )
    domain_height: List[float] = Field(
        ..., description="Height of the simulation domain in each dimension"
    )
...
class SimulationConfig(BaseModel):
    ...
    grid_shape: List[int] = Field(
        ..., min_items=3, max_items=3, description="Number of grid points (nx, ny, nz)"
    )
    ...
    domain_lo: Tuple[float, float, float] = Field(
        ..., description="Lower corner of the domain (x0, y0, z0)."
    )
    ...
    field_manager: FieldManagerConfig = Field(
        default_factory=FieldManagerConfig,
        description="Configuration for the FieldManager.",
    )
    amr: AMRConfig = Field(
        default_factory=AMRConfig, description="Adaptive mesh refinement settings"
    )
    materials: MaterialConfig = Field(
        default_factory=MaterialConfig,
        description="Material selection and initial state",
    )
    ...
    simulation_description: str = Field(
        "A Dense Plasma Focus simulation", description="Description of the simulation"
    )
    # Add other parameters as needed
...
class ServerConfig(BaseModel):
    host: str = "0.0.0.0"
    port: int = Field(
        5000, gt=0, lt=65536, description="Port for the server to listen on"
    )
    admin_username: str = "admin"
    admin_password_hash: str = Field(
        ..., description="Hashed password for the admin user"
    )
    max_simultaneous_simulations: int = Field(
        5, gt=0, description="Maximum number of simultaneous simulations"
    )
    telemetry_interval: float = Field(
        0.1, gt=0, description="Interval for sending telemetry data [s]"
    )
    data_directory: str = Field(
        "data", description="Directory to store simulation data"
    )
    cpu_time_limit: int = Field(
        60, gt=0, description="CPU time limit per simulation in seconds"
    )
    memory_limit: int = Field(
        1024**3, gt=0, description="Address space limit per simulation in bytes"
    )
    # Add other parameters as needed
...
class SheathConfig(BaseModel):
    ...
    electron_density: float = Field(
        ..., description="Electron density at the sheath edge"
    )
    ...
    plasma_edge_potential: float = Field(
        0.0, description="Potential at the plasma edge"
    )
    opacity_model: str = Field("constant", description="Model for opacity calculation")
    opacity_params: Dict[str, Any] = Field(
        default_factory=dict, description="Parameters for opacity model"
    )
    secondary_emission_coefficient: float = Field(
        0.0, description="Secondary electron emission coefficient"
    )
...
class TurbulenceConfig(BaseModel):
    ...
    sigma_epsilon: float = Field(
        1.3, gt=0, description="Turbulent Prandtl number for epsilon"
    )
    ...
    wall_function_type: str = Field(
        "none", description="Type of wall function to use (none, log_law, power_law)"
    )
    compressibility_correction: bool = Field(
        False, description="Enable compressibility correction"
    )
    wall_function_kappa: float = Field(
        0.41, description="Von Karman constant for wall functions"
    )
    wall_function_E: float = Field(
        9.8, description="Empirical constant for wall functions"
    )
    compressibility_alpha: float = Field(
        1.0, description="Compressibility correction coefficient"
    )
    compressibility_beta: float = Field(
        1.0, description="Compressibility correction coefficient"
    )
    # Add other parameters as needed
