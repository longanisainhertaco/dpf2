...
class FieldManager:
    ...
    def update_E(self, new_E: np.ndarray):
        ...
        if new_E.shape != self.E.shape:
            raise ValueError(
                f"Invalid shape for E: expected {self.E.shape}, got {new_E.shape}"
            )
        self.E = new_E
    ...
    def update_B(self, new_B: np.ndarray):
        ...
        if new_B.shape != self.B.shape:
            raise ValueError(
                f"Invalid shape for B: expected {self.B.shape}, got {new_B.shape}"
            )
        self.B = new_B
    ...
    # ------------------------------------------------------------------
    def _yee_derivative(
        self, f: np.ndarray, axis: int, h: float, bc_lo: str, bc_hi: str
    ) -> np.ndarray:
    ...
    def apply_boundary_conditions(self, state: "SimulationState"):
        ...
        for name, field in (("E", self.E), ("B", self.B), ("J", self.J)):
            if field.shape != expected_shape:
                raise ValueError(
                    f"{name} has invalid shape {field.shape}; expected {expected_shape}"
                )
    ...
    def deposit_current(self, current_density: np.ndarray):
        ...
        if current_density.shape != self.J.shape:
            raise ValueError(
                f"Invalid shape for current_density: expected {self.J.shape}, got {current_density.shape}"
            )
...
class SimulationState:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.boundary_conditions = boundary_conditions
        self.density = (
            density if density is not None else np.zeros(grid_shape)
        )  # Ensure density is initialized
    ...
    def __str__(self):
        ...
        return (
            f"SimulationState(density={self.density.shape if self.density is not None else None}, "
    ...
    def add_particles(
        self, name: str, q: float, m: float, pos: np.ndarray, vel: np.ndarray
    ):
        ...
        if name in self.species:
            ...
            self.species[name] = {"q": q, "m": m, "pos": pos.copy(), "vel": vel.copy()}
    ...
    def sample_positions(self, n: int) -> np.ndarray:
        ...
        pts = [(x0 + x[i] * Lx, y0 + y[i] * Ly, z0 + z[i] * Lz) for i in range(n)]
        return np.array(pts)
