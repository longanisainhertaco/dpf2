...
np_stub = types.SimpleNamespace(
    array=lambda x, dtype=None: (
        [float(v) for v in x] if isinstance(x, (list, tuple)) else float(x)
    ),
    ndarray=list,
    zeros=lambda n: (
        [0.0] * n if isinstance(n, int) else [[0.0] * n[1] for _ in range(n[0])]
    ),
...
spec = importlib.util.spec_from_file_location(
    "dpf2.radiation.multigroup", base / "radiation" / "multigroup.py"
)
...
spec2 = importlib.util.spec_from_file_location(
    "dpf2.physics.mhd", base / "physics" / "mhd.py"
)
...
def test_resistive_mhd_balances_loss_and_gain():
    ...
    rad_gain = sum(
        sum(g_new) - sum(g_old) for g_old, g_new in zip(rad_before, rad.energy)
    )
    assert math.isclose(fluid_loss, rad_gain)
