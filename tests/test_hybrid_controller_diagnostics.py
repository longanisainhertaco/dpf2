...
scipy_stub.__path__ = []
scipy_stub.ndimage = types.SimpleNamespace(
    gaussian_filter=lambda a, sigma: a, label=lambda m: (np.zeros_like(m), 0)
)
scipy_stub.constants = types.SimpleNamespace()
scipy_stub.interpolate = types.SimpleNamespace(
    interp1d=lambda *a, **k: None, RegularGridInterpolator=lambda *a, **k: None
)
...
config = types.SimpleNamespace(
    criteria=types.SimpleNamespace(
        grad_thr=0.1, knud_thr=0.1, hall_thr=0.1, non_max_fac=1.0
    ),
    coupling=types.SimpleNamespace(
        buffer_cells=1,
        filter_sigma=1,
        blend_width=1,
        max_iters=1,
...
def test_diagnostic_histories(monkeypatch):
    hc = hc_mod.HybridController(
        ...
        DummyModule(),
        DummyModule(),
        types.SimpleNamespace(get_J=lambda: 0.0),
    )
    monkeypatch.setattr(hc, "apply_boundary_conditions", lambda state, dt: None)
    monkeypatch.setattr(
        hc, "compute_collision_frequency", lambda state: np.zeros((1, 1, 1))
    )
