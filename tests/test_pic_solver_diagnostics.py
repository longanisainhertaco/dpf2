...
def test_boundary_conditions():
    ...
    solver = PICSolver(cfg, fm)
    solver.add_species(
        "e", -1.0, 1.0, np.array([[3.9, 0.0, 0.0]]), np.array([[0.5, 0.0, 0.0]])
    )
    E = np.zeros_like(fm.E)
    B = np.zeros_like(fm.B)
    spc = solver.species["e"]
    solver.boris_push_numba(
        spc["pos"],
        spc["vel"],
        spc["q"],
        spc["m"],
...
def test_model_hooks_called():
    fm = make_field_manager()
    cfg = make_config(
        {"x": "reflecting", "y": "reflecting", "z": "reflecting"},
        dt=0.1,
        quantum=True,
        radiation=True,
        adapt=True,
    ...
    solver = PICSolver(cfg, fm)
    solver.add_species(
        "e", -1.0, 1.0, np.array([[0.0, 0.0, 0.0]]), np.array([[0.0, 0.0, 0.0]])
    )
    calls = []
